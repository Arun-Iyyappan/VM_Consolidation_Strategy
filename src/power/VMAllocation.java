...
public class VMAllocation 
{
    ...
    public void readVM()
    {
        try
        {
            File fe=new File("C:\\Users\\w136648\\Downloads\\vmcode\\projcode\\vm scheduling pso alo\\vm1.txt");
     ...
     public void readHost()
    {
        try
        {
            File fe=new File("C:\\Users\\w136648\\Downloads\\vmcode\\projcode\\vm scheduling pso alo\\host2.txt");
            FileInputStream fis=new FileInputStream(fe);
            byte bt[]=new byte[fis.available()];
            fis.read(bt);
            fis.close();
           ...
           Random rn=new Random();
            for(int i=0;i<dt.pop;i++)
            {
                ...
                for(int j=0;j<dt.vmlist.size();j++)
                {
                    ...
                    int s1=rn.nextInt(dt.hostList.size());
                    PowerHost ht=dt.hostList.get(s1);
                    while(ht.isSuitableForVm(vm))
                    {
                        s1=rn.nextInt(dt.hostList.size()-1);
